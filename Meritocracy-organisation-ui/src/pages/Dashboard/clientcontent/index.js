...
class Clientcontent extends React.Component {
  constructor(props) {
    ...
    app.getprofile(function(result) {
      var state = self.state
  ...
  login = event => {
    app.authorlogin(event.currentTarget.dataset.id, function(value) {})
  }

  author = event => {
    var self = this
    app.author(event.currentTarget.dataset.id, function(result) {
      var state = self.state
      state.userprofile = result
      console.log(state)
      self.setState(state)
      ...
      },
      {
        ...
        render: (text, record) => {
          if (
            ...
            this.state.userprofile.organization.indexOf(record.clientId) > -1
          ) {
            return (
              <Button type="primary" onClick={this.login} data-id={record.clientId}>
                Login
              </Button>
            )
          } else {
            return (
              <Button type="primary" onClick={this.author} data-id={record.clientId}>
                Author
              </Button>
            )
          }
