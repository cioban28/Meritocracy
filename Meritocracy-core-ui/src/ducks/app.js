...
export async function login(username, password, dispatch) {
  ...
  try {
    ...
    if (hash.length > 1) {
      var hashurl = hash[1].split('&')
      if (hashurl[0].split('=')[1]) {
        window.location.href =
          hashurl[0].split('=')[1] + '?username=' + username + '&role=administrator'
        return
      }
    }
...
export async function createclient(clientid, protocol, rootUrl) {
  var result = await AuthAPI.createclient(clientid, protocol, rootUrl)
  if (result.data.error) {
    notification.open({
      type: 'error',
      message: result.data.error_description,
