...
function getclientbyid(clientid,callback){
    adminClient.getclientbyid(clientid).then(function(result){
        callback(result);
    }).catch(function(result){
        callback(result);
    })
...
function organizationlogin(username,password,clientid,secret,callback)
{
    var verb = 'POST';
    var path = '/auth/realms/master/protocol/openid-connect/token';
    var data = {
    ...
    var headers = {
        'Content-Type': 'application/x-www-form-urlencoded',
        'authorization': 'Basic ' + btoa(data.client_id + ':' + secret)
    };
    const requestOptions = {
        headers: headers,
        url: 'http://localhost:8080' + path,
...
function getclientsecret(id,callback){
    adminClient.getclientsecret(id).then(function(result){
        callback(result);
    })
    .catch(function (result) {
        callback(result);
